...
class TestChatMessageRoutes:
    ...
    async def test_send_message_rate_limited_returns_429(self, async_client, db_session):
        ...
        try:
            ...
            assert "Try again in" in third.json()["detail"]
        finally:
    ...
    async def test_rate_limit_is_scoped_per_user_and_session(self, async_client, db_session):
        ...
        try:
            creator = await self._create_user(
                db_session,
                "msg-rate-scope-creator@example.com",
                "msg_rate_scope_creator",
                role="creator",
            )
            participant = await self._create_user(
                db_session,
                "msg-rate-scope-participant@example.com",
                "msg_rate_scope_participant",
                role="consumer",
            )
            other_nano = await self._create_published_nano(db_session, creator.id)
            base_nano = await self._create_published_nano(db_session, creator.id)
            session_one = await self._create_session(
                db_session,
                base_nano.id,
                creator.id,
                participant.id,
            )
            session_two = await self._create_session(
                db_session,
                other_nano.id,
                creator.id,
                participant.id,
            ...
            first_in_session_one = await async_client.post(
                f"/api/v1/chats/{session_one.id}/messages",
                headers=headers,
                json={"content": "Scope test #1"},
            )
            second_in_session_one = await async_client.post(
                f"/api/v1/chats/{session_one.id}/messages",
                headers=headers,
                json={"content": "Scope test #2"},
            )
            first_in_session_two = await async_client.post(
                f"/api/v1/chats/{session_two.id}/messages",
                headers=headers,
                json={"content": "Scope test different session"},
            )

            assert first_in_session_one.status_code == 201
            assert second_in_session_one.status_code == 429
            assert first_in_session_two.status_code == 201
        finally:
            CHAT_MESSAGE_RATE_LIMITER.max_requests = original_max_requests
            CHAT_MESSAGE_RATE_LIMITER.window_seconds = original_window_seconds
            CHAT_MESSAGE_RATE_LIMITER.reset()
    ...
    @pytest.mark.asyncio
    async def test_send_message_content_filter_blocks_phishing_domain(
        self, async_client, db_session
    ):
        """Content filter rejects phishing domains with 400 and reason detail."""
        creator = await self._create_user(
            db_session,
            "msg-filter-creator@example.com",
            "msg_filter_creator",
            role="creator",
        )
        participant = await self._create_user(
            db_session,
            "msg-filter-participant@example.com",
            "msg_filter_participant",
            role="consumer",
        )
        nano = await self._create_published_nano(db_session, creator.id)
        session = await self._create_session(db_session, nano.id, creator.id, participant.id)
        ...
        response = await async_client.post(
            f"/api/v1/chats/{session.id}/messages",
            headers={"Authorization": f"Bearer {participant_token}"},
            json={"content": "Check https://spam.ru/win-money-now"},
        )
