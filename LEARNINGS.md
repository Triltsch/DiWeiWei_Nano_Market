...
- Moderations-Transitions auf zulässige Zielzustände beschränken: `FeedbackModerationRequest` sollte `"pending"` als Zieli ablehnen (Moderatoren können nur `"approved"` oder `"hidden"` setzen); der pendende Zustand wird ausschließlich durch Content-Updates ausgelöst.
